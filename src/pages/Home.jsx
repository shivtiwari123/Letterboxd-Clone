import { Box, Container, Flex, Grid, Heading, Skeleton } from "@chakra-ui/react"
import { useEffect, useState } from "react"
import { fetchTrending, imagePath } from "../services/api"
import CardComponent from "../components/CardComponent";

const Home = () => {

    const [data, setData] = useState([]);
    const [timeWindow, setTimeWindow] = useState('day');
    const [loading, setLoading] = useState(true);
    useEffect(() => {
        setLoading(true);
      fetchTrending(timeWindow)
        .then((res) => {
        setData(res);
      })
      .catch((err) => {
        console.log(err, "err");
      }).finally(() => {
        setLoading(false);
      })
    }, [timeWindow]);
         
    console.log(data, 'data')
  return (
    <Container maxW={"container-xl"}>
        <Flex alignItems={"baseline"} gap={"4"} my={"10"}>
            <Heading color={"white"} as="h2" fontSize={"md"} textTransform={"uppercase"}>Trending</Heading>
            <Flex alignItems={"center"} gap={"2"} border={"1px solid teal"} borderRadius={"20px"}>
                <Box color={"white"} as="button" px="3" py="1" bg={`${timeWindow === 'day' ? "gray.600" : ""}`} borderRadius={"20px"} onClick={() => setTimeWindow("day")}>Today</Box>
                <Box color={"white"} as="button" px="3" py="1" bg={`${timeWindow === 'week' ? "gray.600" : ""}`} borderRadius={"20px"} onClick={() => setTimeWindow("week")}>This Week</Box>
            </Flex>
        </Flex>

        {/* {loading && <div>Loading...</div>} */}
        <Grid templateColumns={{
            base: "1fr",
            sm: "repeat(2, 1fr)",
            md: "repeat(4, 1fr)",
            lg: "repeat(5, 1fr)"
        }} gap="4">  
            {data && data?.map((item, i) => 
                // <img key={item?.id} src={`${imagePath}/${item?.poster_path}`} />
                loading ? (
                    <Skeleton height={300} color={"black"} key={i} />
                ) : (
                    <CardComponent key={item?.id} item={item} type={item?.media_type}/>
                )
            )}
        </Grid> 
    </Container>
  )
}

export default Home